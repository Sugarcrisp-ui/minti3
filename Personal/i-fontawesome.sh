...
install_font_awesome() {
    local zip_file="$dest_dir/$(basename "$1")"

    # Download Font Awesome
    wget -P "$dest_dir" "$1" || handle_error "Failed to download Font Awesome archive."

    # Unzip Font Awesome to source directory
    unzip -q "$zip_file" -d "$dest_dir" || handle_error "Failed to unzip Font Awesome archive."

    # Move the fonts to /usr/share/fonts/
    sudo mv "$dest_dir"/*-desktop /usr/share/fonts/ || handle_error "Failed to move Font Awesome fonts."

    # Change ownership to $USER:$USER
    sudo chown -R $USER:$USER /usr/share/fonts/fontawesome-free* || handle_error "Failed to change ownership for Font Awesome."
...
rm -f "$dest_dir/fontawesome-free-6.5.1-desktop.zip"
rm -f "$dest_dir/fontawesome-free-5.15.4-desktop.zip"
rm -rf "$dest_dir/fontawesome-free-6.5.1-desktop"
rm -rf "$dest_dir/fontawesome-free-5.15.4-desktop"
